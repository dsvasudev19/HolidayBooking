...
const DEMO_STAY_LISTINGS = __stayListing.map((post, index): StayDataType => {
  ...
  return {
    ...post,
    vendor:{name:'',avatar:faker.image.avatar()},
    property_typeId:1,
    propertyMedia:[{path:faker.image.avatar()}],
    featuredPropertyMedia:{path:faker.image.avatar()},
    room:[],
    property:{
      description:'',
      address:'',
      ...
      title:'',
      property_typeId:1,
      propertyMedia:[{path:faker.image.avatar(),url:'',file_name:''}],
      featuredPropertyMedia:{path:faker.image.avatar(),url:'',file_name:''},
      room:[]
    },
